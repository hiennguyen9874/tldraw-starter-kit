...
export function proposeCanvasActions(
	...
	const actionSources = new Map<string, { index: number; fieldPrefix: string }>()
	...
	for (const [index, action] of actions.entries()) {
		...
		if (action.type === 'create') {
			...
			actionSources.set(action.item.id, { index, fieldPrefix: 'item' })
		...
		if (action.type === 'update') {
			...
			actionSources.set(action.id, { index, fieldPrefix: 'patch' })
	...
	if (!parsed.success) {
		const firstIssue = parsed.error.issues
			.map((issue) => {
				...
				const source = item ? actionSources.get(item.id) : undefined
				return {
...
function removeItem(
	...
	changedIds: string[],
	deletedIds: string[],
	actionSources: Map<string, { index: number; fieldPrefix: string }>,
	actionIndex: number
) {
	items.delete(id)
	...
	for (const item of [...items.values()]) {
		if (item.type === 'arrow' && (item.fromId === id || item.toId === id)) {
			...
			actionSources.set(item.id, { index: actionIndex, fieldPrefix: 'id' })
